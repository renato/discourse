...
import { $createRawNode } from "../../nodes/raw-node";
import { $createUnknownNode } from "../../nodes/unknown-node";

// TODO Extract transformers to separate files

// TODO: this is a temporary solution to avoid quotes with inner paragraphs
//       being hard to escape; we're just avoiding adding the paragraph within quotes
const SKIP_INNER_PARAGRAPH = ["quote"];
...
const DEFAULT_TRANSFORMERS = {
  ...
  paragraph_open: (parser) => {
    if (!SKIP_INNER_PARAGRAPH.includes(parser.top().getType())) {
      parser.openNode($createParagraphNode());
    }
  },
  paragraph_close: (parser) => {
    if (!SKIP_INNER_PARAGRAPH.includes(parser.top().getType())) {
      parser.closeTopNode();
    }
  },
